...
Every number this skill uses lives here. Skills and reference files cite a key
...
If you need a number that is not here, add it here first, then cite it.
...
| `coverage.awareness_spread` | **at most one awareness level per ad set** | [ours] | See `intake-and-avatar.md`. Revealing to someone who already knows is condescending; confirming to someone who does not know is meaningless. Creatives aimed at different levels belong in different ad sets, or in one ad set and one level. |
...
| `scrape.max_pull` | **60 ads per run** | [ours] | A cost and attention gate, not a data gate. Past sixty nobody reads the output, and the download volume starts to matter. Ask before exceeding it. |
...
| `scrape.self_exclusion` | **the user's own brand is excluded before any pattern is counted** | [ours] | Requires knowing the brand name. This is one of the three blocking questions in `intake-and-avatar.md`. |
...
Read `video-ads.md` for what to do with these. They exist to separate two
...
| Key | Value | Tag | Notes |
|---|---|---|---|
| `voice.worth_it_rule` | **a voice-over earns its cost only when the script carries information the picture cannot** | [ours] | A voice reading the on-screen text aloud is a wasted channel. See `video-ads.md`. |
