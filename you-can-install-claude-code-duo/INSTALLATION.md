# Installing `meta-ads-team-codex-and-claude-code` for Claude Code
...
| Location | Scope |
|---|---|
| `<project>/.claude/skills/meta-ads-team-codex-and-claude-code/` | this project only |
| `~/.claude/skills/meta-ads-team-codex-and-claude-code/` | every project on this machine |

Unzip `INSTALL-claude-code-duo.zip` into one of them. The archive contains the folder,
so `meta-ads-team-codex-and-claude-code/SKILL.md` ends up at the right depth.

```bash
mkdir -p ~/.claude/skills
unzip INSTALL-claude-code-duo.zip -d ~/.claude/skills/
```
