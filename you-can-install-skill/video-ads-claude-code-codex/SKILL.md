...
description: Use for ANY video advertising request when both assistants are wanted: a video ad, a Reel, a TikTok, Shorts or YouTube ad, a UGC script, an ad hook, a storyboard, a motion or animated ad, or re-cutting an ad that already runs. Creates and improves video advertising with Claude Code opening the mission and Codex cross-reviewing through the real Agent Duet bridge, from brief to inspected MP4 files at every requested ratio, with an integrated second brain for discovery, retention research, hooks, scripts, storyboards, voice, music, assembly and campaign learning.
---
...
5. Follow [the production contract](references/production-contract.md) phase by phase and keep one `motion-project.json`, per [the manifest](references/motion-manifest.md). Choose a look with [art direction](references/art-direction.md) rather than applying a house style. Then [video models](references/video-models.md), [prompting](references/video-prompting.md), [voice](references/video-voice.md), [music](references/video-music.md), [assembly](references/video-assembly.md) and [real production](references/live-action.md) when filming beats generating. A still used as a layer is prompted with [image prompting](references/image-prompting.md), which carries the slot order, the canvases a model will accept and the dated model deprecations, so one side writes the prompt and the other reads it back against the product truth before a generation is paid for. Every requested ratio gets its own composition, and the mandatory ratios are the ones in the brief. A reference engine is supplied and runs: `python scripts/render_motion.py motion-project.json --root . --apply --contact-sheet` turns the manifest into real files, one composition per ratio, no JavaScript required. [The engine](references/motion-engine.md) documents the layer vocabulary, the design tokens, resume after an interruption and its honest limits. Agree which side runs it, and prefer a better renderer whenever one is genuinely available.
