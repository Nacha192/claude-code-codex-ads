...
operating systems, immediately turned that runner red. The cause is worth knowing:
**Homebrew's `ffmpeg 9.0.1` bottle carries libx264 and aac and no libfreetype or
libass**, so `drawtext` and `subtitles` do not exist. Every encode succeeds. Not one
character can be put on screen.

The engine behaved exactly as designed: capability detection named both missing
filters, the run stopped with exit 3, and nothing was written. An engine that trusted
"ffmpeg is installed" would have produced a finished-looking ad with none of the words
in it, at the right duration and the right loudness, passing every technical check.

The tests were the part that was wrong. They treated ffmpeg being on PATH as ffmpeg
being able to render. They now gate on what the build can actually do and skip with
the reason the engine gives, and a new test drives the whole one-shot against a
stubbed crippled machine to prove it refuses rather than shipping a silent ad.

macOS keeps its ffmpeg, because everything that does not draw type now runs there:
the whole inspector, the forward contract, the measurement tests. What skips, skips
for a stated reason rather than because nobody looked.

The same release moves image prompting into the shared trunk. It shipped only in the
still packs, and a motion manifest is full of `image` layers: a product still, a
texture, a backplate. Those are generated by the same model, against the same
canvas rule, and the video packs had no page saying so. The release validator caught
the move being done badly, which is the point of having it: the page closed by
linking to still production, a file that does not exist in a motion pack, so four
packs shipped a broken link for exactly as long as it took to run the validator.

One thing that page now carries and did not before: **a still that will move needs
more pixels than the frame.** A layer with a camera push or a parallax offset shows
more of itself over the scene than the first frame does, so a still generated at
exactly the delivery size runs out of picture mid-shot and exposes what sits behind
it. Nothing measured that, because the first frame is always correct.
