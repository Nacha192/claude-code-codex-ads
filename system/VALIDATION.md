...
- **Those rules are proved by tests rather than by hand.** Eight cases plant a real violation in a throwaway copy of the release and require the validator to refuse it, one per rule, plus a control on a clean copy so that a rule firing on everything cannot pass as a rule that works. Before this, every structural rule had been demonstrated once, manually, and a later regression would have stayed green.
