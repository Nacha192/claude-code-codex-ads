...
| The machine is asked what it can do, and an unusable machine is named rather than discovered halfway through an encode. `brew install ffmpeg` on macOS is exactly that machine: libx264 and aac present, libfreetype absent, so every encode succeeds and not one character can be drawn. The run stops with exit 3 and names the missing filter instead of delivering an ad with the words missing | `test_detection_names_what_is_missing_rather_than_failing_late`, `test_a_complete_build_reports_nothing_missing`, `test_an_ffmpeg_that_cannot_draw_is_named_rather_than_used`, `test_a_machine_that_cannot_draw_stops_instead_of_shipping_a_silent_ad` |
...
| Line widths come out of the font file and are checked against what the renderer actually draws, not against the parser's own arithmetic. A face that cannot be parsed falls back rather than failing | `test_the_measured_width_is_the_width_that_gets_drawn`, `test_the_font_file_is_read_rather_than_assumed`, `test_an_unreadable_face_falls_back_instead_of_raising`, `test_no_wrapped_line_exceeds_the_column_it_was_wrapped_to` |
| Two renders of an unchanged manifest produce identical bytes, so the hash recorded in it is a fact about the project rather than about one run | `test_rendering_the_same_manifest_twice_produces_the_same_bytes`, `test_a_gradient_is_the_same_gradient_on_the_next_run` |
