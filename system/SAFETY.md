...
- The committed packs must be byte-identical to what the committed sources rebuild, checked by `git diff --exit-code` in CI, and a rebuild of unchanged sources must produce identical archive bytes and an identical `SHA256SUMS`. `test_building_twice_produces_the_same_bytes`
