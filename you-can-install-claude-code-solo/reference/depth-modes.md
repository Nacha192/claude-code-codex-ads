...
|---|---|---|
| Model | `Opus 5` | `Opus 5, maximum reasoning effort` |
...
`Both modes run on the same model here. The depth comes from the reasoning effort and from the six passes below, not from a different model, so the difference is entirely in work performed. Never claim a depth mode whose passes produced no artefact.`
...
```
Mode: second brain (Opus 5, maximum reasoning effort) · passes 1-6 complete · ledger read and updated
```

or

```
Mode: standard (Opus 5) · six checks only · escalate if <the trigger that applies>
```
