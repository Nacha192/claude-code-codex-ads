...
def validate():
 ...
 for name in NAMES:
  ...
  zpath=ROOT/'dist'/f'install-{name}.zip'
 ...
 if len(list((ROOT/'dist').glob('*.zip')))!=4:errors.append('Expected four ZIPs')
 ...
 print(json.dumps({'errors':errors,'skill_count':len(skills),'source_count':len(source),'zip_count':len(list((ROOT/'dist').glob('*.zip')))},indent=2))
 return bool(errors)
