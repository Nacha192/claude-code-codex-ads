...
and `generation-tools.md` for the gate that stands in front of all of them.
...
drift. Generate 3 to 6 second beats and cut them together, and write the edit so
the inevitable drift lands on a cut.
...
| Text on the label | garbled glyphs | generate without text, typeset it after. See `static-ads.md` |
...
Add the voice deliberately, per `voice.md`, and the music per `video-music.md`.
