...
|---|---|---|
| Model | `{{STANDARD_MODEL}}` | `{{DEEP_MODEL}}` |
...
```
Mode: second brain ({{DEEP_MODEL}}) · passes 1-6 complete · ledger read and updated
```

or

```
Mode: standard ({{STANDARD_MODEL}}) · six checks only · escalate if <the trigger that applies>
```
