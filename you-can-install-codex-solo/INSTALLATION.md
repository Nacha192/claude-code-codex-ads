# Installing `meta-ads-codex` for Codex
...
| Location | Scope |
|---|---|
| `<repo>/.agents/skills/meta-ads-codex/` | this repository, and therefore the team |
| `~/.codex/skills/meta-ads-codex/` | every project on this machine |

Unzip `INSTALL-codex-solo.zip` into one of them. The archive contains the folder,
so `meta-ads-codex/SKILL.md` ends up at the right depth.
...
```markdown
To build Meta ads, load the meta-ads-codex skill.
```
